...
app.get("/signatures", async (req, res) => {
  const text = "SELECT * from signatures LIMIT 100";
  const signatures = await client.query(text); //FIXME-TASK: get signatures from db!
...
app.get("/signatures/:id", async (req, res) => {
  ...
  const text = "SELECT * from signatures WHERE id = $1 RETURNING *";
  const value = [id];
  const signature = await client.query(text, value); //FIXME-TASK get the signature row from the db (match on id)
...
app.post("/signatures", async (req, res) => {
  ...
  if (typeof name === "string") {
    const text =
      "INSERT INTO signatures(signature, message)VALUES($1,$2) RETURNING *";
    const value = [name, message];
    const createdSignature = await client.query(text, value); //FIXME-TASK: insert the supplied signature object into the DB
...
app.put("/signatures/:id", async (req, res) => {
  ...
  if (typeof name === "string") {
    const text =
      "UPDATE signatures SET signature = $1, message= $2 WHERE id = $3 RETURNING *";
    const value = [name, message, id];
    const result: any = await client.query(text, value); //FIXME-TASK: update the signature with given id in the DB.
...
app.delete("/signatures/:id", async (req, res) => {
  ...
  const value = [id];
  const queryResult: any = await client.query(text, value); ////FIXME-TASK: delete the row with given id from the db
